...
if gsc_file and sf_file:
    ...
    with st.expander("📋 Required File Formats"):
        ...
        1. Go to Configuration > Custom > Extractionimport streamlit as st
import pandas as pd
import numpy as np
from typing import Union, List
import io
...
# Helper functions
def load_file(file):
    """Load CSV or Excel file into pandas DataFrame with robust CSV parsing."""
    if file.name.endswith('.csv'):
        # Reset file pointer to the beginning, just in case
        file.seek(0)
        try:
            # Use the more flexible 'python' engine and specify how to handle bad lines.
            # 'on_bad_lines='warn'' will print a warning for each bad line but continue.
            return pd.read_csv(file, engine='python', on_bad_lines='warn', sep=',')
        except Exception as e:
            st.error(f"Failed to parse CSV file: {file.name}. Error: {e}")
            st.warning("This can happen if commas exist inside your data. The tool attempted a robust parse but failed. Please check the file for formatting issues.")
            return None
    elif file.name.endswith(('.xlsx', '.xls')):
        try:
            return pd.read_excel(file, engine='openpyxl' if file.name.endswith('.xlsx') else 'xlrd')
        except Exception as e:
            st.error(f"Failed to parse Excel file: {file.name}. Error: {e}")
            return None
    else:
        raise ValueError("Unsupported file format")
...
if gsc_file and sf_file:
    gsc_df, crawl_df = None, None

    # Load GSC data with specific error handling
    try:
        with st.spinner("Loading Google Search Console data..."):
            gsc_df = load_file(gsc_file)
    except Exception as e:
        st.error(f"An error occurred while loading the GSC file: {e}")
        st.write("Please ensure the GSC file is a valid CSV or Excel file and not corrupted.")

    # Load Crawl data with specific error handling
    try:
        with st.spinner("Loading Screaming Frog data..."):
            crawl_df = load_file(sf_file)
    except Exception as e:
        st.error(f"An error occurred while loading the Screaming Frog file: {e}")
        st.write("Please ensure the Screaming Frog file is a valid CSV or Excel file and not corrupted.")
            ...
            if processed_gsc is not None and len(processed_gsc) > 0:
                ...
                if processed_crawl is not None:
                    ...
                    with col1:
                        st.metric("Total URLs Analyzed", len(report))
                    with col2:
                        total_keywords = report['Keywords in Striking Distance'].sum()
                        st.metric("Total Keywords in Striking Distance", int(total_keywords))
                    with col3:
                        total_clicks = report['Total Clicks (Top Keywords)'].sum()
                        st.metric("Total Clicks Potential", int(total_clicks))
                    ...
                    st.error("Failed to process Screaming Frog data. Please check the file format.")
            else:
                st.warning("No keywords found in the specified position range after filtering.")
                
        except Exception as e:
            st.error(f"An error occurred during data processing: {str(e)}")
            st.write("Please check that your files have the correct format and columns as described in the instructions.")
            st.info("If you continue to experience issues, try exporting your data again or contact support.")
